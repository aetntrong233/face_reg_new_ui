...
def feature_extractor(inputs):
	feature_extractor_layers = InceptionResNetV2_keras(input_shape=(299, 299, 3), include_top=False, weights='imagenet')
...
def classifier(inputs, num_class):
...
def get_train_model(num_class):
	...
	classification_output = classifier(res_feature_extractor, num_class)
...
def get_pretrained_model(model_path):
	...
	output = BatchNormalization(momentum=0.995, epsilon=0.001, scale=False)(emb_layer)
...
def get_model(weights_path):
	inputs = Input(shape=(299, 299, 3))
	res_model = InceptionResNetV2()
	res_model.trainable = False
	res_feature_extractor = res_model(inputs)
